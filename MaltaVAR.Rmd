...
(b) Model fit (trend, seasonality, cycle) and discussion HPI MALTA
```{r,}
...
(b) Model fit and discussion EUGDP
```{r}
...
Despite qualitiative asessment of residuals vs fitted values seeming to gravitate away from zero, the acf and pacf of the residuals demonstrate that the residuals resemble a white noise series, therefore suggesting that the model is robust enough to move forward with. The Box-test also concludes with enough evidence to fail to reject H-nought, therefore compounding the claim that the residuals are independently distributed.
...
The EUGDP model seems stable and does not have significant structural breaks.
...
The ACF and the PACF suggest that the residuals don't exhibit any serial correlation.

The residuals of EUGDP look to be randomly distributed about a mean of 0, however, there seems to be some outliers at an index of around 15.
...
(h) 12-step ahead forecast
```{r}
plot(forecast(mEU2 , h=12 ),shadecols="oldstyle", main="EU GDP ARIMA Forecast",xlab="Year", ylab ="Index")
```
